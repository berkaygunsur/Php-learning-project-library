...
error_reporting(0);
 include($_SERVER['DOCUMENT_ROOT'].'/baglan.php');
  ...
  $favoriListeleSql = "SELECT * FROM kitaplar WHERE favori = '1'";
  $favoriListele = $db->prepare($favoriListeleSql);
  $favoriListele->execute();

  echo " <div id='favoriTablosuDiv'>
     <table id='favoriTablosu'>
      <tr id='favoriTablosuBaslik'>
        <th>Yazar</th>
        <th>Kitap</th>
        <th>Raf</th>
        <th>Konum</th>
  ...
  foreach ($favoriListele as $bilgi) {
    ...
    echo "<script>
    var table = document.getElementById('favoriTablosu');
